...
parser.add_argument('--rnn_size', default=16, type=int, help='rnn_size.')
...
parser.add_argument('--adj_data',
                    type=str,
                    default='data/sensor_graph/adj_NYC.csv',
                    help='adj data path')
...
def main(runid):
    ...
    if args.TEST_ONLY:
        ...
        for i in range(1, args.epochs + 1):
            ...
            for iter, (x, y, ycl) in enumerate(
                ...
                metrics = engine.train(trainx,
                                       ...
                                       trainycl,
                                       idx=None,
                                       batches_seen=batches_seen,epoch=i)
                train_loss.append(metrics[0])
                train_mape.append(metrics[1])
                train_rmse.append(metrics[2])
                train_average_time.append(metrics[3])
            ...
            for iter, (x, y) in enumerate(
                ...
                valid_average_time.append(metrics[3])
            s2 = time.time()
            ...
            mtrain_average_time = np.mean(train_average_time)

            mvalid_loss = np.mean(valid_loss)
            mvalid_mape = np.mean(valid_mape)
            mvalid_rmse = np.mean(valid_rmse)
            mvalid_average_time = np.mean(valid_average_time)
            his_loss.append(mvalid_loss)

            if (i - 1) % args.print_every == 0:
                log = 'Epoch: {:03d}, Inference Time: {:.4f} secs'
                print(log.format(i, (s2 - s1)))
                log = 'Epoch: {:03d}, Train Loss: {:.4f}, Train MAPE: {:.4f}, Train RMSE: {:.4f},Train TIME: {:.4f}, Valid Loss: {:.4f}, Valid MAPE: {:.4f}, Valid RMSE: {:.4f},Valid TIME: {:.4f}, Training Time: {:.4f}/epoch'
                print(log.format(i, mtrain_loss, mtrain_mape, mtrain_rmse,mtrain_average_time,
                                 mvalid_loss, mvalid_mape, mvalid_rmse,mvalid_average_time,
                                 (t2 - t1)),
                      flush=True)
        ...
        for iter, (x, y) in enumerate(dataloader['val_loader'].get_iterator()):
            ...
            testy = testy.transpose(1, 3)
            with torch.no_grad():
                preds, average_time,grad_mask,baselines,log_pi,wait_penalty  = engine.model(testx, ycl=testy)
        #         preds = preds.transpose(1, 3)
        ...
        vmae, vmape, vrmse = metric(pred, realy[:,:,-1])
        ...
        for iter, (x,y) in enumerate(dataloader['test_loader'].get_iterator()):
            ...
            with torch.no_grad():
                ...
                # preds = preds.transpose(1, 3)
            outputs.append(preds)
            average_times.append(average_time.item())
          
        # halt_time=torch.cat(torch.Tensor(average_times),dim=0)
        yhat = torch.cat(outputs, dim=0)
        yhat = yhat[:realy.size(0), ...]
        ...
        for i in [11]:
            ...
            mape.append(metrics[1])
            rmse.append(metrics[2])
        return vmae, vmape, vrmse, mae, mape, rmse, average_times
...
if __name__ == "__main__":
    ...
    mape = np.array(mape)
    rmse = np.array(rmse)
    halt_time=np.array(halt_time)

    amae = np.mean(mae, 0)
    amape = np.mean(mape, 0)
    armse = np.mean(rmse, 0)
    atime=np.mean(halt_time,0)

    smae = np.std(mae, 0)
    smape = np.std(mape, 0)
    srmse = np.std(rmse, 0)
    stime=np.std(halt_time,0)
    ...
    print(
        'test|horizon\tMAE-mean\tRMSE-mean\tMAPE-mean\tTIME-mean\tMAE-std\tRMSE-std\tMAPE-std\tTIME-std'
    )
    for i in range(1):
        log = '{:d}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}'
        print(
            log.format([10][i], amae[i], armse[i], amape[i], atime[i],smae[i],
                       srmse[i], smape[i],stime[i]))
